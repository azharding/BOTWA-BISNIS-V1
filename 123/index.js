...
console.log(color('=> WELCOME TO COREIZEN AI', 'yellow'))
console.log(color('=> ADA BUG ATAU ERROR INI NO ADMIN', 'yellow'), color('6285262677553'))
console.log(color(`faxx.json`), color(`BOT TERSAMBUNG`, 'red'))
    let { version, isLatest } = await fetchLatestBaileysVersion()
    const mans = mansConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['COREIZEN','Safari','1.0.0'],
        auth: state,
        version
    })
...
require("./coreizen")(mans, m, chatUpdate, store)
} catch (err) {
console.log(err)
}
})
//GRUP UPDATE
mans.ev.on('groups.update', async pea => {
       //console.log(pea)
    // Get Profile Picture Group
       try {
       ppgc = await mans.profilePictureUrl(pea[0].id, 'image')
       } catch {
       ppgc = 'https://telegra.ph/file/3983c55ac7f3ebea225d3.jpg'
       }
       let wm_fatih = { url : ppgc }
       if (pea[0].announce == true) {
       mans.send5ButImg(pea[0].id, `「 Group Settings Change 」\n\nGroup telah ditutup oleh admin, Sekarang hanya admin yang dapat mengirim pesan !`, `Group Settings Change Message`, wm_fatih, [])
...
mans.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    mans.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `Welcome To ${metadata.subject} @${num.split("@")[0]}` })
...
// Setting
	...
	for (let i of kon) {
	    list.push({
	    	...
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await mans.getName(i + '@s.whatsapp.net')}\nFN:${await mans.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:aldistore@gmail.com@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/Aldistore840\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
